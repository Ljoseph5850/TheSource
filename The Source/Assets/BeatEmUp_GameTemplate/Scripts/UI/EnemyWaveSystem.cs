...
public class EnemyWaveSystem : MonoBehaviour {
	...
	[SerializeField]
	public int currentWave, levelToGive, levelToSet;
	...
	private RoundClearScrn theRoundClearScrn;
	private GameOverScrn theGameOverScrn;
	
	public bool settingRound;
	...
	void Start(){
		...
		theRoundClearScrn = FindObjectOfType<RoundClearScrn>();
		theGameOverScrn = FindObjectOfType<GameOverScrn>();
	}
	
	void Update(){
		if(settingRound){
			theRoundClearScrn.setTry();
		}
	}

	public void OnLevelStart(){
		if(onLevelStart != null) onLevelStart();
		theRoundClearScrn.activateLevelSelect(levelToGive);
		theGameOverScrn.activateLevelSelect(levelToSet);
		StartWave();
	}
