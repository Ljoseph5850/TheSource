...
public class PlayerCombat : MonoBehaviour {
	...
	private void Update(){
		...
		if(Input.GetKeyDown(KeyCode.Space)&& Input.GetKeyDown(KeyCode.B) && Input.GetKeyDown(KeyCode.N)){
			Debug.Log("Combo Started");
		}

		if(Input.GetKeyDown(KeyCode.T)){
			if(Input.GetKeyDown(KeyCode.H) && Input.GetKeyDown(KeyCode.G)){
			Debug.Log("Double Combo Started");
			}
		}
		...
		RageSystem ps = GetComponent<RageSystem>();
		if(ps != null){
			ps.GetRage(rageStatus);
		}

		if(rageStatus < -20){
			rageStatus = -6;
		}else if(rageStatus > 100){
			rageStatus = 100;
		}
	}
	...
	private void CombatInputEvent(string action) {
		if (AttackStates.Contains (playerState.currentState) && !isDead) {
			...
			if (action == "Kick" && playerState.currentState != PLAYERSTATE.PUNCH && (rageStatus > 5)) {
			...
			}else if (action == "Kick" && playerState.currentState != PLAYERSTATE.PUNCH && (rageStatus < 5)){
				FluidAnimator.TriggerFluidEmptyAnimation();
			}
	...
	public void ThrowKnife() {
		...
		int dir = (int)GetComponent<PlayerMovement> ().getCurrentDirection ();
		knife.transform.position = transform.position + Vector3.up * .2f + Vector3.right * dir * .7f;
	...
	private void Hit(DamageObject d) {
		if (!HitStates.Contains (playerState.currentState)) {
			...
			//play hit SFX
			if(wasHit) GlobalAudioPlayer.PlaySFX ("PlayerHurt");
