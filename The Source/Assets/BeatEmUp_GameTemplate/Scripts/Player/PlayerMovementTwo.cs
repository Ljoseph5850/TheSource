...
public class PlayerMovementTwo : MonoBehaviour
{
	...
	public DIRECTIONAlso currentDirection;
	...
	void Awake() {
		...
		currentDirection = DIRECTIONAlso.Right;
		isGrounded = true;
	}
	...
	void Move(Vector3 vector) {
		if (playerState.currentState != PLAYERSTATE.JUMPING && playerState.currentState != PLAYERSTATE.JUMPKICK) {
			...
			if (Mathf.Abs (vector.x + vector.y) > 0) {
				...
				currentDirection = (DIRECTIONAlso)i;
				animator.Walk();
	...
	public void LookToDir(DIRECTIONAlso dir) {
		if (dir == DIRECTIONAlso.Left)
			GFX.transform.localScale = new Vector3(-1,1,1);
		else if (dir == DIRECTIONAlso.Right)
			GFX.transform.localScale = new Vector3(1,1,1);
	}
	...
	public DIRECTIONAlso getCurrentDirection() {
